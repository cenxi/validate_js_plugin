(function(){
                $.regex = {
                    rules: {
                        ...
                        intRange: {
                            validator: function (str, range) {
                                return (/^\d+$/.test(str))&&str>=range[0]&&str<=range[1];
                            },
                            message: "请填入{0} - {1}范围内的值。"
                        }
                    },
                };
                $.extend({
                    validateExe:function () {
                        $.message = "";
                        $.errorInput={};
                        $('input[validType]').each(function () {
                            var input=this;
                            var val=$(this).val();
                            var validType = $(this).attr('validType');
                            $.each(validType.split(';'), function () {
                                var type=this.split('[')[0];
                                if(!!this.split('[')[1]) {
                                    var params=this.split('[')[1].substr(0,this.split('[')[1].length-1);
                                    var param=[];
                                    $.each(params.split(','),function () {
                                        param.push(parseInt(this));
                                    });
                                    console.log(param)
                                    if(!$.regex.rules[type].validator(val,param)){
                                        $.message=$.regex.rules[type].message;
                                        var i=0;
                                        $.each(param,function(){
                                            $.message=$.message.replace("{" + i + "}", this);
                                            i++;
                                        })
                                        input.focus();
                                        $.errorInput=input;
                                    ...
                                    if(!$.regex.rules[type].validator(val)){
                                        $.message=$.regex.rules[type].message;
                                        input.focus();
                                        $.errorInput=input;
                                        return false;
